...
// calculateTeamOverall still used for the live box-score display while drafting
...
function DraftContent() {
  ...
  const roundLabel = allFilled ? 'Completo' : `Round ${filledCount + 1} / 11`;
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      // 1 chamada: gera calendário + simula 38 jogos + persiste tudo no banco
  ...
  return (
    <div className="min-h-screen bg-paper dark:bg-midnight flex flex-col transition-colors relative z-10">

      {/* Header */}
      <header className="border-b border-ink/15 dark:border-gold/15 px-6 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          <div className="border-2 border-gold px-4 py-1 text-[10px] font-black uppercase tracking-widest text-ink dark:text-cream">
            {roundLabel}
          </div>
          <button
            onClick={() => router.push('/')}
      ...
      {/* 3-column body */}
      <div className="flex-1 grid grid-cols-[300px_1fr_260px] overflow-hidden">

        {/* ── LEFT PANEL ── */}
        <div className="border-r border-ink/15 dark:border-gold/10 flex flex-col overflow-hidden bg-parchment dark:bg-navy">

          <div className="flex-1 flex flex-col overflow-hidden">
            {rolledSquad ? (
              <>
                {/* CLUBE + ERA vintage cards */}
                <div className="grid grid-cols-2 gap-3 p-4 border-b border-ink/12 dark:border-gold/10 shrink-0">
                  ...
                  <div className="relative border-2 border-coral p-3 text-center bg-coral/[0.07]">
                    ...
                    <p className="text-[8px] tracking-[0.5em] uppercase text-coral font-bold mb-2 relative z-10">
                      Clube
                    </p>
                    <p className="text-lg font-black text-ink dark:text-cream leading-tight uppercase relative z-10">
                      {rolledSquad.team}
                    </p>
                  </div>
                  {/* ERA */}
                  <div className="relative border-2 border-gold p-3 text-center bg-gold/[0.07]">
                    ...
                    <p className="text-[8px] tracking-[0.5em] uppercase text-gold font-bold mb-2 relative z-10">
                      Era
                    </p>
                    <p className="text-lg font-black text-ink dark:text-cream leading-tight uppercase relative z-10">
                      {rolledSquad.era}
                ...
                <div className="px-4 pt-3 pb-3 border-b border-ink/10 dark:border-gold/10 shrink-0">
                  ...
                  <button
                    ...
                    className="w-full border border-ink/25 dark:border-gold/40 py-2 text-[10px] font-bold uppercase tracking-widest text-ink/75 dark:text-cream/60 hover:border-gold hover:text-ink dark:hover:text-cream disabled:opacity-30 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                  >
                    ↺ Outra Equipe
                  </button>
                </div>

                {/* Player list or position selection */}
                      ...
                      <div className="flex flex-col gap-2">
                        {compatibleSlots.map((i) => (
                          <button
                            ...
                            className="border-2 border-coral bg-coral/[0.07] hover:bg-coral hover:text-cream py-2.5 px-4 text-[12px] font-black uppercase tracking-widest transition-colors text-left flex items-center justify-between text-ink dark:text-cream"
                          >
                            <span>{slots[i].slot_pos}</span>
                            <span className="text-[9px] text-ink/60 dark:text-cream/40 group-hover:text-cream">pos. {i + 1}</span>
                          </button>
                        ))}
                      </div>
                      <button
                        onClick={handleCancelSelection}
                        className="w-full mt-3 text-[9px] text-ink/60 dark:text-cream/30 hover:text-ink dark:hover:text-cream/65 py-2 uppercase tracking-widest transition-colors"
                      >
                        ← Voltar
                      </button>
                    </div>
                      ...
                      {squadPlayers.map((p, idx) => {
                        ...
                        return (
                          <button
                            ...
                            disabled={!canFit}
                            className={`w-full flex items-center gap-3 px-4 py-3 border-b border-ink/8 dark:border-gold/8 text-left transition-colors ${
                              canFit
                                ? 'hover:bg-paper dark:hover:bg-midnight cursor-pointer'
            ...
            {!allFilled ? (
              !rolledSquad ? (
                <div className="relative">
                  ...
                  <button
                    ...
                    className="relative w-full bg-gold text-midnight font-black text-lg uppercase tracking-[0.2em] py-4 disabled:opacity-40 hover:bg-gold/90 hover:shadow-[0_0_28px_rgba(201,168,76,0.55)] transition-all duration-200 border-2 border-gold shadow-[inset_0_0_0_1px_rgba(201,168,76,0.4)]"
                  >
                    ROLAR ◆
                  </button>
                </div>
                ...
                <button
                  onClick={resetSquad}
                  className="w-full border border-ink/30 dark:border-gold/35 text-ink/75 dark:text-cream/60 font-bold text-sm uppercase tracking-widest py-3 hover:border-gold dark:hover:border-gold hover:text-ink dark:hover:text-cream hover:shadow-[0_0_16px_rgba(201,168,76,0.2)] transition-all duration-200"
                >
                  ← Cancelar
                </button>
              )
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="relative w-full bg-green text-cream font-black text-lg uppercase tracking-[0.2em] py-4 hover:bg-green/90 hover:shadow-[0_0_24px_rgba(27,107,58,0.5)] disabled:opacity-40 transition-all duration-200 border-2 border-green"
                >
        ...
        {/* ── CENTER: Field ── */}
        <div className="flex items-center justify-center p-8 bg-paper/50 dark:bg-midnight">
        ...
        {/* ── RIGHT: Box Score ── */}
        <div className="border-l border-ink/15 dark:border-gold/10 p-5 overflow-y-auto bg-parchment dark:bg-navy">
